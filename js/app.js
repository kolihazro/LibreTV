// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["tyyszy","dyttzy", "bfzy", "ruyi","heimuer","dbzy","ffzy","zy360","iqiyi","mozhua","mdzy","wujin","wwzy","ikun"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["tyyszy","dyttzy", "bfzy", "ruyi","heimuer","dbzy"];
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {},
            `Winter - 免费在线视频搜索与观看平台`,
            `/`
        );
        // 更新页面标题
        document.title = `Winter - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - Winter`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
        ...
        if (yellowFilterEnabled) {
            const banned = ['剧情片','伦理','电影解说','日本伦理','韩国伦理','理论片','理伦片','伦理片', '福利', '里番动漫', '门事件', '萝莉少女', '制服诱惑', '国产传媒', 'cosplay', '黑丝诱惑', '无码', '日本无码', '有码', '日本有码', 'SWAG', '网红主播', '色情片', '同性片', '福利视频', '福利片'];
        ...
        if (data.episodes && data.episodes.length > 0) {
            ...
            modalContent.innerHTML = `
                ...
                <div class="flex flex-wrap items-center justify-between mb-4 gap-2">
                    ...
                    <button  class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm transition-colors">
                        点击下方选集播放
                    </button>
                </div>
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'Winter-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'Winter-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'Winter-Settings_' + times + '.json');
}
